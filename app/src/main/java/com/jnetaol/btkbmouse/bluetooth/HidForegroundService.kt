...
class HidForegroundService : Service() {
    companion object {
        ...
        const val SDP_DESC = "Keyboard and Mouse via Bluetooth"
    ...
    var onStatus: ((Boolean, Boolean, String?) -> Unit)? = null
    var onDevice: ((BluetoothDevice?) -> Unit)? = null
        ...
        override fun onReceive(c: android.content.Context?, i: Intent?) {
            when (i?.action) {
                BluetoothAdapter.ACTION_STATE_CHANGED -> {
                    if (i.getIntExtra(BluetoothAdapter.EXTRA_STATE, -1) == BluetoothAdapter.STATE_ON) start()
                    else { running = false; emit() }
                }
                BluetoothDevice.ACTION_BOND_STATE_CHANGED -> {
                    if (i.getIntExtra(BluetoothDevice.EXTRA_BOND_STATE, -1) == BluetoothDevice.BOND_BONDED) start()
                }
            }
        }
    }
    ...
    override fun onCreate() {
        ...
        NotificationChannel(CHANNEL_ID, "BT HID", NotificationManager.IMPORTANCE_LOW).apply {
    ...
    override fun onStartCommand(i: Intent?, f: Int, id: Int): Int {
        startForeground(NOTIFY_ID, notif("Starting..."))
    ...
    private fun register() {
        ...
        try {
            ...
            if (ok) { registered = true; attempts = 0; emit() }
            else retry(3000L)
        } catch (e: Exception) { retry(5000L) }
    }
    ...
    private fun retry(d: Long) {
        if (!running) return
        handler.removeCallbacksAndMessages(null)
        handler.postDelayed({ start() }, d.coerceIn(2000L, 30000L))
    }

    private val cb = object : BluetoothHidDevice.Callback() {
        override fun onAppStatusChanged(dev: BluetoothDevice?, reg: Boolean) {
            registered = reg
            if (reg && dev != null) { device = dev; onDevice?.invoke(dev) }
            else if (!reg) { device = null; onDevice?.invoke(null); if (running) retry(3000L) }
            emit()
        }
        override fun onConnectionStateChanged(dev: BluetoothDevice?, s: Int) {
            if (s == BluetoothProfile.STATE_CONNECTED) { device = dev; onDevice?.invoke(dev); emit() }
            else if (s == BluetoothProfile.STATE_DISCONNECTED && dev == device) {
                device = null; onDevice?.invoke(null); emit()
            }
        }
    ...
    private fun emit() {
        onStatus?.invoke(registered, device != null, null)
        notif(if (device != null) "Connected: ${device?.name}" else if (registered) "Ready - scan BT on host" else "Starting...")
    }

    private fun notif(txt: String): Notification {
        try {
            getSystemService(NotificationManager::class.java).notify(NOTIFY_ID, Notification.Builder(this, CHANNEL_ID)
    ...
    fun sendKeyboardReport(mods: Byte, keys: ByteArray = byteArrayOf()) {
        val h = hid ?: return; val d = device ?: return
        try {
            val r = ByteArray(8); r[0] = mods
            keys.forEachIndexed { i, k -> if (i < 6) r[2 + i] = k }
            h.sendReport(d, RID_KB.toInt(), r)
        } catch (_: Exception) {}
    }
